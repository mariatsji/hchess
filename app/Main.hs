...
findDepth :: String -> Int -> [String] -> Int
findDepth prefix def params =
  fromMaybe def $ viaNonEmpty head [d | d <- [0..9], (prefix <> show d) `elem` params]

findWhite :: [String] -> Int
findWhite = findDepth "w" 4

findBlack :: [String] -> Int
findBlack = findDepth "b" 4

findStyle :: [String] -> IO Style
findStyle params = pure if "bright" `elem` params then brightTheme else darkTheme

startFromPos :: [String] -> IO (Maybe Position)
startFromPos params = do
